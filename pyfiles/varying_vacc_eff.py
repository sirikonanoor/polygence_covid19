...
# all the vaccine effectiveness that are to be tested
vaccine_eff = np.arange(start = 0.0,
                        stop = 1.1,
                        step = 0.1)
...
# to measure time taken per sim, store starting time
t = time.time()
...
for sim in range(num_sims):
    ...
    # update the percent infected population after each sim
    data[sim, 1] = percent_infected
    
    maximum = np.max(reff[num_people, :])
    final_reff[sim] = maximum
...
# calculate protective effect offered by each vaccine effectiveness
protective_effect = np.mean(data[0:5, 1]) - data[:, 1]
# add protective effect to data matrix
data = np.insert(data, 2, protective_effect, axis = 1)
# add reff ot data matrix
